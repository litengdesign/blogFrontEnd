...
export class PostsComponent implements OnInit {
  ...
  public api_list = '/api/postsManage/list'; // 页面数据列表api
  public api_add = '/api/postsManage/add'; // 新增行api
  public api_update = '/api/postsManage/update'; // 更新行api
  public api_delete = '/api/postsManage/delete'; // 删除行api
  public rowId = '';
  ...
  editRow(item) {
    ...
    let initformData = new Observable<any>((observer) => {
      ...
      this.validateForm.setValue({ name: item.name, status: item.status, isTop: item.isTop, description: item.description, content: item.content, category: selectCategory });
      observer.next();
    })
